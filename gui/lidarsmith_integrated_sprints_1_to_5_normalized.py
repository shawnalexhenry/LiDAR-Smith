...
class LidarSmithApp:
    def __init__(self, root):
        self.root = root
        self.root.title("LiDAR/Smith - Integrated v0.2 (Preview Normalized)")
        self.array = None
        self.mask = None
        self.coords = None
        self.fmt = None
    ...
    def setup_gui(self):
        ttk.Button(self.root, text="Load Terrain File", command=self.load_file).pack(pady=2)

        ttk.Label(self.root, text="Spike Filter Level:").pack()
        self.level_menu = ttk.Combobox(self.root, values=["Light", "Medium", "Heavy", "Aggressive"], state="readonly")
        self.level_menu.set("Medium")
        self.level_menu.pack(pady=2)

        ttk.Label(self.root, text="Smoothing Settings").pack()
        ttk.Label(self.root, text="Median Kernel Size (odd):").pack()
        self.median_menu = ttk.Combobox(self.root, values=["Off", "3", "5", "7", "9"], state="readonly")
        self.median_menu.set("Off")
        self.median_menu.pack(pady=2)

        ttk.Label(self.root, text="Gaussian Sigma (0.0–3.0):").pack()
        self.gaussian_slider = tk.Scale(self.root, from_=0.0, to=3.0, resolution=0.1, orient=tk.HORIZONTAL)
        self.gaussian_slider.set(0.0)
        self.gaussian_slider.pack(pady=2)

        self.preview_var = tk.IntVar()
        ttk.Checkbutton(self.root, text="Show Preview", variable=self.preview_var, command=self.update_preview).pack(pady=2)

        ttk.Button(self.root, text="Run Full Pipeline", command=self.run_pipeline).pack(pady=2)
        ttk.Button(self.root, text="Save Cleaned Terrain", command=self.save_output).pack(pady=2)
        ttk.Button(self.root, text="Save Coordinate Log", command=self.save_coords).pack(pady=2)
